 ...
 *   1. Creates the Scryloft Organisation when the table is empty.
...
describe("Phase 1 migration: 001-organisations", () => {
  ...
  test("creates Scryloft Organisation when the table is empty", async () => {
    ...
    expect(orgs[0].name).toBe(migration.SCRYLOFT_DEFAULT_NAME);
    expect(orgs[0].leadEmployeeId).toBe(migration.SCRYLOFT_DEFAULT_LEAD);
    expect(orgs[0].wipCap).toBe(migration.SCRYLOFT_DEFAULT_WIP_CAP);
  });
  ...
  test("moves a legacy org/ directory into organisations/<id>/org/", async () => {
    ...
    writeYaml(path.join(legacyOrg, "engineering", "lead-leon.yaml"), {
      name: "lead-leon",
      displayName: "Leon",
    ...
    expect(fs.existsSync(path.join(newOrgRoot, "org", "engineering", "lead-leon.yaml"))).toBe(true);
    ...
    expect(indexed.map((e) => e.name)).toContain("lead-leon");
  });
