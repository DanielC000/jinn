...
describe("Phase 5 task-bound sessions", () => {
  ...
  test("findChildSessionByEmployeeAndTask returns existing live session", async () => {
    ...
    const found = registry.findChildSessionByEmployeeAndTask("lead-aaron", task.id);
  ...
  test("archived sessions don't count for per-task uniqueness", async () => {
    ...
    expect(registry.findChildSessionByEmployeeAndTask("lead-aaron", task.id)).toBeUndefined();
  });
  ...
  test("different tasks produce different sessions for the same employee", async () => {
    ...
    expect(registry.findChildSessionByEmployeeAndTask("backend-henrik", tA.id)?.id).toBe(a.id);
    expect(registry.findChildSessionByEmployeeAndTask("backend-henrik", tB.id)?.id).toBe(b.id);
  });
});
